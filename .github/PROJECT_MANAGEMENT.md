...
- 毎朝、Projectsの `Estimate (h)` ・`予定日` ・`Status`（値が `本日作業` のものだけ）を集計し、担当者ごとの合計時間を算出
...
- `Status = 本日作業` の条件を加えているのは、「予定日が今日」というだけで実際にはまだ着手確定していないタスクを除外するため。朝会でその日やると決めたタスクだけを対象にできる
